 ...
 */
export const syncRoleUserUpdate = async (roleDocId, updateData, role) => {
    const RoleModel = RoleModels[role];
    if (!RoleModel)
        throw new Error(`Invalid role: ${role}`);
    // 1. Update Role Collection
    const roleDoc = await RoleModel.findByIdAndUpdate(roleDocId, { $set: updateData }, { new: true, runValidators: true }).select('-password');
    if (!roleDoc)
        throw new Error('Role document not found');
    // 2. Sync User Collection (same ID)
    const userDoc = await User.findByIdAndUpdate(roleDocId, { $set: updateData }, { new: true }).select('-password');
    return {
        roleDoc,
        userDoc,
        synced: true
 ...
 */
export const syncRoleUserDelete = async (roleDocId, role) => {
    const RoleModel = RoleModels[role];
    if (!RoleModel)
        throw new Error(`Invalid role: ${role}`);
    // 1. Soft delete Role Collection
    const roleDoc = await RoleModel.findByIdAndUpdate(roleDocId, {
        $set: {
            status: 'inactive',
            deletedAt: new Date()
        }
    ...
    await User.findByIdAndUpdate(roleDocId, {
        $set: {
            status: 'inactive',
            deletedAt: new Date()
        }
...
export const syncRoleUserStatus = async (roleDocId, newStatus, role) => {
    const RoleModel = RoleModels[role];
    if (!RoleModel)
        throw new Error(`Invalid role: ${role}`);
    // Update Role status
...
export const getLinkedUserByRoleId = async (roleDocId, role) => {
