...
export const getLeaves = async (req, res) => {
    try {
        const { page = 1, limit = 10, search = '', status, type: leaveType, sort = 'createdAt' } = req.query;
        const query = {};
        ...
        const leaves = await Leave.find(query)
            ...
            .sort(sort === 'recent' ? { createdAt: -1 } : { fromDate: 1 })
            .limit(Number(limit))
...
export const updateLeaveStatus = async (req, res) => {
    try {
        ...
        await auditLog(req, {
            ...
            details: `Leave request ${status.toLowerCase()} for ${leave.employeeId?.name || "Employee"}`,
