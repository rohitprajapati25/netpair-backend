...
export const loginUser = async (req, res) => {
    try {
        ...
        if (!isMatch) {
            await auditLoginLog(req, { id: user._id.toString(), name: user.name, role: user.role || "unknown", email }, false, `Failed login — wrong password for ${email}`);
        ...
        const token = jwt.sign({ id: user._id, role: tokenRole }, process.env.JWT_SECRET, { expiresIn: "30m" });
        // ── Audit: successful login ──────────────────────────────────────────────
        await auditLoginLog(req, { id: user._id.toString(), name: user.name, role: tokenRole, email: user.email }, true, `${user.name} (${tokenRole}) logged in successfully`);
        res.status(200).json({
