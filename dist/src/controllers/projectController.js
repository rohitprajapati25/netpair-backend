...
export const getProjects = async (req, res) => {
    try {
        console.log('📋 GET PROJECTS - Request received, user:', req.user);
...
export const createProject = async (req, res) => {
    try {
        ...
        const projectData = {
            ...
            projectCode,
            createdBy: user.id,
            ...(req.files ? { attachments: req.files.map((f) => ({
...
export const deleteProject = async (req, res) => {
    ...
    try {
        ...
        const taskResult = await Task.updateMany({ project_id: projectId, deletedAt: null }, { deletedAt: new Date() }, { session });
        ...
        const timesheetResult = await Timesheet.updateMany({ project_id: projectId, deletedAt: null }, { deletedAt: new Date() }, { session });
        console.log('✅ Timesheets deleted:', timesheetResult.modifiedCount);
        // Soft delete Project
        console.log('📁 Deleting project...');
        const projectResult = await Project.findByIdAndUpdate(projectId, { deletedAt: new Date() }, { session, new: true });
