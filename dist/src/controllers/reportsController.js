...
export const getUnifiedReports = async (req, res) => {
    try {
        ...
        const unifiedStats = {
            totalEmployees: statsResults[0].status === 'fulfilled' ? statsResults[0].value : 0,
            statsResults, [1]: .status === 'fulfilled' ? statsResults[1].value.find((s) => s._id?.toLowerCase() === 'present')?.count || 0 : 0,
            totalAssets: statsResults[2].reduce((sum, s) => sum + (s.count || 0), 0),
            statsResults, [3]: .status === 'fulfilled' ? statsResults[3].value.find((s) => s._id?.toLowerCase() === 'ongoing')?.count || 0 : 0,
            pendingLeaves: statsResults[4].find((s) => s._id?.toLowerCase() === 'pending')?.count || 0,
            tasksCompleted: statsResults[5].find((s) => s._id?.toLowerCase() === 'completed')?.count || 0,
            timesheetsPending: statsResults[6].find((s) => s._id?.toLowerCase() === 'pending')?.count || 0
        };
