...
export const protect = async (req, res, next) => {
    ...
    try {
        ...
        if (!user) {
            ...
            if (role === "employee") {
                user = (await Employee.findById(decoded.id).select("status role email name").lean());
            }
            else if (role === "hr") {
                user = (await HR.findById(decoded.id).select("status role email name").lean());
            }
            else if (role === "admin") {
                user = (await Admin.findById(decoded.id).select("status role email name").lean());
            }
        }
        if (!user) {
            return res.status(401).json({ success: false, message: "User not found" });
        }
        const normalizedStatus = user.status?.toString().toLowerCase().trim() || "inactive";
        ...
        const userPayload = {
            id: String(user._id),
            role: (user.role || role).toLowerCase().trim(),
