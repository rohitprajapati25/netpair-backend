...
router.get("/audit-logs", protect, authorizeRoles(ROLES.SUPER_ADMIN), async (req: any, res: any) => {
  try {
    const {
      ...
      search = '', action = 'All', severity = 'All', status = 'All',
    } = req.query;
...
router.delete("/audit-logs/:id", protect, authorizeRoles(ROLES.SUPER_ADMIN), async (req: any, res: any) => {
  try {
...
router.delete("/audit-logs", protect, authorizeRoles(ROLES.SUPER_ADMIN), async (req: any, res: any) => {
  try {
