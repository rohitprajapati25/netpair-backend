...
const parseDevice = (ua: string = ""): { name: string; os: string; browser: string; type: string } => {
  if (!ua || ua === "—") {
  ...
  else if (/axios/i.test(ua))         browser = "Axios (API)";

  // ── Device type ──────────────────────────────────────────────────────────────
  let type = "Desktop";
  if      (/Mobile|Android.*Mobile|iPhone/i.test(ua)) type = "Mobile";
  else if (/iPad|Tablet|Android(?!.*Mobile)/i.test(ua)) type = "Tablet";
...
export const auditLoginLog = async (
  req: Request,
