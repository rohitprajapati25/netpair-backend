...
const calcWorkingMins = (checkIn: string, checkOut: string): number => {
  const inMins  = timeToMins(checkIn);
  const outMins = timeToMins(checkOut);
  let diff = outMins - inMins;
  if (diff < 0) diff += 1440; // overnight (rare)
  return diff;
};

/** Determine status from checkIn time */
const deriveStatus = (checkInTime: string): string => {
  const inMins = timeToMins(checkInTime);
  if (inMins > SHIFT_START_MINS) return "Late";   // after 09:00 = Late
  return "Present";
};
...
export const checkOut = async (req: Request, res: Response) => {
  try {
    ...
    const updated = await Attendance.findByIdAndUpdate(
      record._id,
      { checkOut: checkOutTime, workingHours, ...(notes && { notes }), updatedBy: userId },
      { new: true }
    ).populate("employee", "name department designation");
