...
const seedWeekAttendance = async () => {
  try {
    await connectDb();
    console.log('🌱 Seeding CURRENT WEEK attendance data...');
      ...
      for (let empIdx = 0; empIdx < 5; empIdx++) {
        ...
        for (let shift = 0; shift < 2; shift++) {
          records.push({
            ...
            workingHours: Math.floor(Math.random() * 4 + 6) * 60, // 6-10 hours
    ...
    console.log(`✅ Seeded ${records.length} attendance records for current week (Sun-${dayOffset} days)`);
    console.log('🔥 Now Reports page "week" filter will show ~70 records!');
    process.exit(0);
  } catch (err) {
